...
class TestEC2Key:
    ...
    def test_ec2_key_hpke_ec_p256(self, kdf, aead):
        suite = HPKECipherSuite(kem=0x0010, kdf=kdf, aead=aead)
        ...
        assert b"Hello world!" == priv_key.open(suite, enc, ciphertext)
        with pytest.raises(ValueError) as err:
            pub_key.open(suite, b"", b"")
            pytest.fail("Public key cannot be used for HPKE decryption.")
        assert "." in str(err.value)
        with pytest.raises(ValueError) as err:
            priv_key.seal(suite, b"")
            pytest.fail("Private key cannot be used for HPKE encryption.")
        assert "." in str(err.value)
    ...
    def test_ec2_key_hpke_ec_p256_unsupported(self, kdf, aead):
        suite = HPKECipherSuite(kem=0x0010, kdf=kdf, aead=aead)
        pub_key = COSEKey.from_jwk(
            {
                "kty": "EC",
        ...
        with pytest.raises(ValueError) as err:
            pub_key.seal(suite, b"Hello world!")
            pytest.fail("seal should fail.")
        assert "Unsupported kdf/aead combination." in str(err.value)
    ...
    def test_ec2_key_hpke_ec_p256_with_invalid_kem(self, kem):
        suite = HPKECipherSuite(kem=kem, kdf=0x0001, aead=0x0001)
        pub_key = COSEKey.from_jwk(
            {
                "kty": "EC",
                "kid": "01",
        ...
        with pytest.raises(ValueError) as err:
            pub_key.seal(suite, b"Hello world!")
            pytest.fail("seal should fail.")
        assert "KEM id should be 0x0010." in str(err.value)
    ...
    def test_ec2_key_hpke_ec_p384_unsupported(self, kdf, aead):
        suite = HPKECipherSuite(kem=0x0011, kdf=kdf, aead=aead)
        pub_key = COSEKey.from_jwk(
            {
                "kty": "EC",
                "crv": "P-384",
    ...
    def test_ec2_key_hpke_ec_p384_with_invalid_kem(self, kem):
        suite = HPKECipherSuite(kem=kem, kdf=0x0001, aead=0x0001)
        pub_key = COSEKey.from_jwk(
            {
                "kty": "EC",
        ...
        with pytest.raises(ValueError) as err:
            pub_key.seal(suite, b"Hello world!")
            pytest.fail("seal should fail.")
        assert "KEM id should be 0x0011." in str(err.value)
    ...
    def test_ec2_key_hpke_ec_p521(self, kdf, aead):
        suite = HPKECipherSuite(kem=0x0012, kdf=kdf, aead=aead)
        priv_key = COSEKey.from_jwk(
            {
                "kty": "EC",
                "crv": "P-521",
    ...
    def test_ec2_key_hpke_ec_p521_unsupported(self, kdf, aead):
        suite = HPKECipherSuite(kem=0x0012, kdf=kdf, aead=aead)
        pub_key = COSEKey.from_jwk(
            {
                "kty": "EC",
    ...
    def test_ec2_key_hpke_ec_p521_with_invalid_kem(self, kem):
        suite = HPKECipherSuite(kem=kem, kdf=0x0001, aead=0x0001)
        pub_key = COSEKey.from_jwk(
            {
                "kty": "EC",
        ...
        with pytest.raises(ValueError) as err:
            pub_key.seal(suite, b"Hello world!")
            pytest.fail("seal should fail.")
        assert "KEM id should be 0x0012." in str(err.value)

    def test_ec2_key_hpke_ec_with_invalid_crv(self):
        ...
        with pytest.raises(ValueError) as err:
            pub_key.seal(suite, b"Hello world!")
            pytest.fail("seal should fail.")
        assert "Invalid crv for HPKE." in str(err.value)
    ...
    def test_ec2_key_hpke_with_key_ops(self, key_ops):
        suite = HPKECipherSuite(kem=0x0010, kdf=0x0001, aead=0x0001)
        priv_key = COSEKey.from_jwk(
            {
                "kty": "EC",
